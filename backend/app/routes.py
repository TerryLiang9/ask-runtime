...
from app.auth import AUTH_COOKIE_NAME, DEMO_SIGNED_OUT_COOKIE_NAME, create_session_token
...
def get_current_user(request: Request):
    ...
    if not container.auth_settings.enabled:
        return container.get_current_user()
    try:
        return container.get_user_by_session(request.cookies.get(AUTH_COOKIE_NAME, ""))
    except PermissionError as exc:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=str(exc)) from exc


def verify_internal_request(request: Request) -> None:
    expected_key = os.getenv("EMATA_INTERNAL_API_KEY", "").strip()
    if not expected_key:
        return
    provided_key = request.headers.get("X-EMATA-INTERNAL-KEY", "").strip()
    if not hmac.compare_digest(provided_key, expected_key):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="internal_unauthorized")


def serialize_user(user) -> MeResponse:
    return MeResponse(
        id=user.id,
...
@public_router.post("/auth/login", response_model=AuthSessionResponse)
def login(
    ...
    if not container.auth_settings.enabled:
        ...
        try:
            user = container.authenticate_user(payload.username, payload.password)
        except PermissionError as exc:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=str(exc)) from exc
    token, expires_at = create_session_token(user.id, container.auth_settings)
    response.set_cookie(
        key=AUTH_COOKIE_NAME,
        value=token,
        httponly=True,
        secure=container.auth_settings.cookie_secure,
        samesite="lax",
        max_age=container.auth_settings.session_ttl_seconds,
        path="/",
    )
    response.delete_cookie(key=DEMO_SIGNED_OUT_COOKIE_NAME, path="/", samesite="lax")
    return AuthSessionResponse(user=serialize_user(user), expires_at=expires_at)


@public_router.post("/auth/logout")
def logout(response: Response, container: ServiceContainer = Depends(get_container)) -> dict:
    response.delete_cookie(key=AUTH_COOKIE_NAME, path="/", samesite="lax")
    if not container.auth_settings.enabled:
        response.set_cookie(
            key=DEMO_SIGNED_OUT_COOKIE_NAME,
            value="1",
            httponly=True,
            secure=container.auth_settings.cookie_secure,
            samesite="lax",
...
@public_router.post("/feishu/events")
async def receive_feishu_event(
    ...
    try:
        return container.handle_feishu_open_event(payload)
    except RuntimeError as exc:
...
@internal_router.post("/planner/plan", response_model=PlannerResponse, dependencies=[Depends(verify_internal_request)])
def build_plan(
...
@internal_router.post("/knowledge/retrieve", response_model=KnowledgeSearchResponse, dependencies=[Depends(verify_internal_request)])
...
@internal_router.post("/connectors/execute", response_model=ConnectorExecuteResponse, dependencies=[Depends(verify_internal_request)])
...
@internal_router.post(
    "/feishu/delivery/{delivery_id}/retry",
    response_model=DeliveryJobResponse,
    dependencies=[Depends(verify_internal_request)],
)
def retry_delivery(
