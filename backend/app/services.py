...
from app.ask.context import AskContextManager
...
from app.feishu_events import FeishuEventHandler, FeishuReplyClient
...
from app.knowledge.ingestion import ChunkPolicyEngine, DoclingParserAdapter
from app.knowledge.models import KnowledgeChunkRecord, KnowledgeSourceFile, UploadStatus
from app.knowledge.rag import AnswerGenerationService, RerankProvider
...
class ServiceContainer:
    def __init__(self, database_url: str = None, temporal_runtime: Optional[TemporalRuntime] = None) -> None:
        ...
        self.auth_settings = load_auth_settings()
        self.user_password_hashes: Dict[str, str] = {}
        self._configure_auth_users()
        ...
        self.feishu_reply_client = FeishuReplyClient()
        self.feishu_event_handler = FeishuEventHandler(self, self.feishu_reply_client)
    ...
    def get_user_by_session(self, token: str) -> UserRecord:
        from app.auth import read_session_user_id

        user_id = read_session_user_id(token, self.auth_settings)
        if not user_id or user_id not in self.store.users:
            raise PermissionError("unauthenticated")
        return self.store.users[user_id]

    def authenticate_user(self, username: str, password: str) -> UserRecord:
        user = find_user_by_username(self.store.users, username)
        if user is None:
            raise PermissionError("invalid_credentials")
        password_hash = self.user_password_hashes.get(user.id, "")
        if not verify_password(password, password_hash):
            raise PermissionError("invalid_credentials")
        return user

    def _configure_auth_users(self) -> None:
        validate_launch_config(self.auth_settings)
        if not self.auth_settings.enabled:
            return
        users, password_hashes = load_bootstrap_users()
        self.store.users = users
        self.user_password_hashes = password_hashes
    ...
    def handle_feishu_open_event(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        self.feishu_event_handler.reply_client = self.feishu_reply_client
        return self.feishu_event_handler.handle(payload)
