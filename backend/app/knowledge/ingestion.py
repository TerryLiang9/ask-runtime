...
from app.knowledge.models import CanonicalBlock, KnowledgeChunkRecord
...
class MinerUPdfParserAdapter:
    ...
    @staticmethod
    def _validate_pdf_file(file_path: str) -> None:
        try:
            ...
            # MinerU can still provide a clearer runtime error for PDFs that pypdf cannot parse.
            return
...
class PypdfPdfParserAdapter:
    def parse_file(self, file_path: str) -> List[CanonicalBlock]:
        MinerUPdfParserAdapter._validate_pdf_file(file_path)
        try:
            reader = PdfReader(file_path)
        ...
        for page_index, page in enumerate(reader.pages, start=1):
            ...
            blocks.append(
                CanonicalBlock(
                    ...
                    metadata={"source_type": "pdf", "parser": "pypdf"},
                )
            )
        return blocks


class FallbackPdfParserAdapter:
    def __init__(
        self,
        mineru_parser: Optional[MinerUPdfParserAdapter] = None,
        pypdf_parser: Optional[PypdfPdfParserAdapter] = None,
    ) -> None:
        self.mineru_parser = mineru_parser or MinerUPdfParserAdapter()
        self.pypdf_parser = pypdf_parser or PypdfPdfParserAdapter()

    def parse_file(self, file_path: str) -> List[CanonicalBlock]:
        try:
            return self.mineru_parser.parse_file(file_path)
        except ValueError as exc:
            if str(exc) != "mineru_executable_not_found":
                raise
        return self.pypdf_parser.parse_file(file_path)
