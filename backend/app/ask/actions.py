...
from app.ask.tools import LarkCliError
...
class AskActionDraftModule:
    ...
    def handle_turn(
        ...
        *,
        ...
        if draft.get("target_resolution_mode") == "explicit_quoted":
            exact_target = self.target_resolver.resolve_exact_candidate(
                query=draft["target_query"],
                user=user,
                tools=tools,
                preferred_kind=self._preferred_target_kind(draft),
            )
            if exact_target:
                draft["resolved_target"] = exact_target
                return self._build_preview_result(draft=draft)
    ...
    def _build_message_draft(self, *, message: str, active_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "target_resolution_mode": self._target_resolution_mode(message),
    ...
    def _build_preview_card(*, draft: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "data": {
                ...
                "available_commands": ["approve_plan", "cancel"],
            },
        }
    ...
    def _build_preview_result(self, *, draft: Dict[str, Any]) -> Dict[str, Any]:
        return {
            ...
            "artifacts": [
                {
                    "artifact_type": "action_preview",
                    "title": draft.get("summary", "动作预览"),
                    "payload": {
                        "intent": draft.get("intent", ""),
                        "risk_level": draft.get("risk_level", "medium"),
                        "summary": draft.get("summary", ""),
                        "target_query": draft.get("target_query", ""),
                        "resolved_target": draft.get("resolved_target", {}),
                        "actions": draft.get("actions", []),
                        "editable_fields": draft.get("editable_fields", []),
                    },
                }
            ],
        }
    ...
    def _detect_action_intent(cls, message: str) -> str:
        ...
        if has_message_marker and has_meeting_marker and re.search(r"(?:并|同时).*(?:发|发送|通知)", content):
            return ""
    ...
    @staticmethod
    def _preferred_target_kind(draft: Dict[str, Any]) -> str:
        hint = str(draft.get("target_type_hint") or "").strip()
        if hint in {"chat", "user"}:
            return hint
        target_query = str(draft.get("target_query") or "").strip()
        if target_query.endswith(("群", "群聊")):
            return "chat"
        return ""

    @staticmethod
    def _target_resolution_mode(message: str) -> str:
        if re.search(r'(?:给|向)[“"].+?[”"]', message or ""):
            return "explicit_quoted"
        return ""
