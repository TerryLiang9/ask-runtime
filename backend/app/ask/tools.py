...
class LarkCliRunner:
    ...
    def _resolve_timeout_seconds() -> float:
        raw = os.getenv("EMATA_LARK_CLI_TIMEOUT_SECONDS", "10").strip()
        try:
            value = float(raw)
        except ValueError:
            return 10.0
    ...
    def ensure_initialized(self, user: UserRecord) -> Path:
        app_id = os.getenv("EMATA_LARK_APP_ID", "").strip()
        app_secret = os.getenv("EMATA_LARK_APP_SECRET", "").strip()
    ...
    def remove_config(self, user: UserRecord) -> None:
        config_dir = self.config_dir_for(user)
        command = self.base_command() + ["config", "remove"]
        self._run_subprocess(
            command,
            env=self._build_env(config_dir),
            input_text=None,
    ...
    def _run_subprocess(
        ...
        *,
        ...
        try:
            ...
            raise LarkCliError(
                "feishu_cli_timeout",
                f"lark-cli command timed out after {self.timeout_seconds:g} seconds.",
                details={"timeout_seconds": self.timeout_seconds, "command": command},
            ) from exc
...
class FeishuBindingService:
    ...
    def get_status(self, user: UserRecord) -> Dict[str, Any]:
        ...
        if record is None:
            ...
            if (config_dir / "config.json").exists():
                ...
                try:
                    ...
                    if self._is_cli_unavailable_error(exc):
                        return self._mark_cli_unavailable(recovered, exc)
        ...
        if record.status in {"ACTIVE", "REAUTH_REQUIRED"}:
            try:
                ...
                if self._is_cli_unavailable_error(exc):
                    return self._mark_cli_unavailable(record, exc)
    ...
    @staticmethod
    def _is_cli_unavailable_error(exc: LarkCliError) -> bool:
        return exc.code in {"feishu_cli_not_found", "feishu_cli_timeout"}
    ...
    def _mark_cli_unavailable(self, record: FeishuBindingRecord, exc: LarkCliError) -> Dict[str, Any]:
        record.status = "UNAVAILABLE"
        if exc.code == "feishu_cli_timeout":
            record.hint = (
                "lark-cli 响应超时，飞书工具暂时不可用；"
                "普通 Ask 和知识问答可以继续使用。"
            )
        else:
            record.hint = (
                "当前环境没有安装 lark-cli，飞书工具暂时不可用；"
                "普通 Ask 和知识问答可以继续使用。"
            )
        record.granted_scopes = []
        record.missing_scopes = list(self.required_scopes())
