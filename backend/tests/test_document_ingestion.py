...
class DocumentIngestionModelTestCase(unittest.TestCase):
    ...
    def test_docling_adapter_surfaces_pdf_runtime_error_when_mineru_is_missing(self) -> None:
        ...
        self.assertEqual(str(context.exception), "invalid_pdf_file")

    def test_parser_registry_falls_back_to_pypdf_when_mineru_is_missing(self) -> None:
        registry = DocumentParserRegistry()
        registry._parsers["pdf"].mineru_parser.executable = "missing-mineru-binary"
        with tempfile.NamedTemporaryFile("wb", suffix=".pdf", delete=False) as pdf_handle:
            pdf_handle.write(self._build_text_pdf_bytes())
            pdf_path = pdf_handle.name

        try:
            blocks = registry.parse_file(pdf_path, "pdf")
        finally:
            os.remove(pdf_path)
        ...
        self.assertEqual(blocks[0].metadata["parser"], "pypdf")
        self.assertIn("Expense policy requires approval", blocks[0].text)
