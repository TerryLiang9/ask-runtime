...
class AskApiTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_feishu_binding_start_returns_unavailable_when_cli_is_missing(self) -> None:
        def missing_cli_subprocess(*args, **kwargs):
            del args, kwargs
        ...
        with patch.dict(
            os.environ,
            {
                "EMATA_LARK_APP_ID": "cli_test_app",
                "EMATA_LARK_APP_SECRET": "cli_test_secret",
                "EMATA_LARK_CLI_EXECUTABLE": "missing-lark-cli",
            },
            clear=False,
        ):
            with patch("app.ask_tools.subprocess.run", side_effect=missing_cli_subprocess):
                client = build_client(preserve_model_env=True)
