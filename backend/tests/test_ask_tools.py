...
class LarkCliRunnerTestCase(unittest.TestCase):
    ...
    def test_run_subprocess_raises_lark_cli_timeout_when_cli_hangs(self) -> None:
        runner = LarkCliRunner()
        command = ["npx.cmd", "-y", "@larksuite/cli", "auth", "status", "--verify"]

        with patch("app.ask_tools.subprocess.run", side_effect=subprocess.TimeoutExpired(command, 1)):
            with self.assertRaises(LarkCliError) as context:
                runner._run_subprocess(
                    command,
                    env={"PATH": "test"},
                    input_text=None,
        ...
        self.assertEqual(context.exception.code, "feishu_cli_timeout")
...
class FeishuBindingServiceTestCase(unittest.TestCase):
    ...
    def test_get_status_reports_unavailable_when_lark_cli_is_missing(self) -> None:
        class MissingCliRunner:
            def config_dir_for(self, user: UserRecord) -> Path:
                return Path("E:/Project/Agent/.runtime/lark-cli") / user.organization_id / user.id

            def run(self, *, user: UserRecord, args, input_text=None, json_expected=True, allow_json_error_result=False):
                del user, args, input_text, json_expected, allow_json_error_result
                raise LarkCliError("feishu_cli_not_found", "lark-cli is not installed or not available.")
        ...
        store.feishu_bindings[user.id] = record
        service = FeishuBindingService(store=store, runner=MissingCliRunner())

        payload = service.get_status(user)

        self.assertEqual(payload["status"], "UNAVAILABLE")
        self.assertIn("lark-cli", payload["hint"])
        self.assertEqual(payload["missing_scopes"], service.required_scopes())

    def test_get_status_reports_unavailable_when_lark_cli_times_out(self) -> None:
        class TimeoutRunner:
            def config_dir_for(self, user: UserRecord) -> Path:
                return Path("E:/Project/Agent/.runtime/lark-cli") / user.organization_id / user.id
            ...
            def run(self, *, user: UserRecord, args, input_text=None, json_expected=True, allow_json_error_result=False):
                del user, args, input_text, json_expected, allow_json_error_result
                raise LarkCliError("feishu_cli_timeout", "lark-cli command timed out.")
        ...
        store.feishu_bindings[user.id] = record
        service = FeishuBindingService(store=store, runner=TimeoutRunner())

        payload = service.get_status(user)

        self.assertEqual(payload["status"], "UNAVAILABLE")
        self.assertIn("超时", payload["hint"])
        self.assertEqual(payload["missing_scopes"], service.required_scopes())

    def test_start_binding_reports_unavailable_when_lark_cli_is_missing(self) -> None:
        class MissingCliRunner:
            def config_dir_for(self, user: UserRecord) -> Path:
                return Path("E:/Project/Agent/.runtime/lark-cli") / user.organization_id / user.id
    ...
    def test_start_binding_force_rebind_removes_cli_config_before_reinitializing(self) -> None:
        class RebindRunner:
            def __init__(self) -> None:
                self.calls = []
            ...
            def remove_config(self, user: UserRecord) -> None:
                del user
            ...
            def run(self, *, user: UserRecord, args, input_text=None, json_expected=True, allow_json_error_result=False):
                ...
                if args[:3] == ["auth", "check", "--scope"]:
                    return {"granted": ["contact:user:search"], "missing": ["search:docs:read"]}
                if args == ["auth", "logout", "--json"]:
                    return {"ok": True}
                if args[:2] == ["auth", "login"] and "--no-wait" in args:
                    return {
        ...
        payload = service.start_binding(user, force_rebind=True)

        self.assertEqual(payload["status"], "PENDING")
        self.assertIn(["config", "remove"], runner.calls)
        self.assertLess(
            runner.calls.index(["config", "remove"]),
            runner.calls.index(["auth", "login", "--json", "--scope", " ".join(service.required_scopes()), "--no-wait"]),
        )
