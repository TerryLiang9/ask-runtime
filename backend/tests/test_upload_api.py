...
class UploadApiTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    @staticmethod
    def _build_text_pdf_bytes(text: str = "Expense policy requires approval.") -> bytes:
    ...
    async def test_upload_endpoint_uses_pypdf_fallback_when_mineru_is_missing(self) -> None:
        app, client, _database_url = build_client()
        app.state.container.document_parser.registry._parsers["pdf"].mineru_parser.executable = "missing-mineru"
        ...
        payload = response.json()
        self.assertEqual(payload["status"], "COMPLETED")
        self.assertEqual(payload["ingestion_summary"]["parser_backend"], "pypdf")
        completed_uploads = [
            item
            for item in app.state.container.store.source_files.values()
            if item.filename == "policy.pdf"
        ]
        self.assertTrue(completed_uploads)
        self.assertEqual(completed_uploads[-1].status.value, "COMPLETED")
        self.assertGreater(app.state.container.get_chunk_count_for_source_file(completed_uploads[-1].id), 0)
        await client.aclose()
