...
function isFeishuSetupError(error) {
  return isOptionalFeishuSetupMessage(error?.message);
}

async function fetchOptionalFeishuBindingStatus() {
  try {
    return await fetchFeishuBindingStatus();
  } catch (error) {
    return buildOptionalFeishuBindingFromError(error.message);
  }
}
...
export default function AskChat({ viewModel }) {
    ...
    async function bootstrap() {
      try {
        ...
        const [bindingPayload, history, artifactPayload] = await Promise.all([
          fetchOptionalFeishuBindingStatus(),
  ...
  async function refreshBinding() {
    const payload = await fetchOptionalFeishuBindingStatus();
  ...
  async function handleStartBinding({ forceRebind = false } = {}) {
    ...
    try {
      ...
      if (isFeishuSetupError(requestError)) {
        updateBindingState(buildOptionalFeishuBindingFromError(requestError.message));
        return;
      }
      setError(formatAskErrorMessage(requestError.message));
    } finally {
  ...
  const bindingBannerClass = getFeishuBindingBannerClass(binding);
  ...
  return (
    <section className="ask-layout">
      <div className="panel ask-chat-panel">
        ...
        <div className={`status-banner ${bindingBannerClass}`}>
