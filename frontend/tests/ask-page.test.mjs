...
import {
  ...
  buildOptionalFeishuBindingFromError,
  ...
  getFeishuBindingBannerClass,
  isOptionalFeishuSetupMessage,
...
test("binding status formatter covers unbound pending and active states", () => {
  ...
  assert.equal(formatFeishuBindingStatus({ status: "UNBOUND" }), "未绑定");
  ...
  assert.equal(formatFeishuBindingStatus({ status: "UNAVAILABLE" }), "飞书未配置");
  assert.equal(
    formatFeishuBindingStatus({
      status: "ACTIVE",
...
test("feishu setup errors degrade to optional integration state", () => {
  assert.match(formatAskErrorMessage("feishu_cli_not_configured"), /普通 Ask/);
  assert.match(formatAskErrorMessage("Feishu CLI credentials are not configured."), /普通 Ask/);
  assert.equal(isOptionalFeishuSetupMessage("Feishu CLI credentials are not configured."), true);

  const fallback = buildOptionalFeishuBindingFromError("Feishu CLI credentials are not configured.");

  assert.equal(fallback.status, "UNAVAILABLE");
  assert.deepEqual(fallback.missing_scopes, []);
  assert.match(fallback.hint, /普通 Ask/);
  assert.equal(getFeishuBindingBannerClass(fallback), "status-idle");
  assert.equal(getFeishuBindingBannerClass({ status: "UNBOUND", missing_scopes: ["im:chat:read"] }), "status-idle");
  assert.equal(getFeishuBindingBannerClass({ status: "ACTIVE", missing_scopes: ["im:chat:read"] }), "status-error");
});
