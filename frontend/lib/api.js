export function resolveApiBaseUrl(env = process.env, { isServer = typeof window === "undefined" } = {}) {
  const browserFallback =
    typeof window !== "undefined" && window.location?.origin ? window.location.origin : "http://localhost:8000";
  const raw = isServer
    ? env.EMATA_API_BASE_URL || env.NEXT_PUBLIC_EMATA_API_BASE_URL || "http://localhost:8000"
    : env.NEXT_PUBLIC_EMATA_API_BASE_URL || env.EMATA_API_BASE_URL || browserFallback;
...
export function shouldUseApiFallbacks(env = process.env) {
  const disabled = String(env.NEXT_PUBLIC_EMATA_DISABLE_API_FALLBACKS || env.EMATA_DISABLE_API_FALLBACKS || "")
    .trim()
    .toLowerCase();
...
function buildCookieHeaders(cookieHeader, headers = {}) {
  if (!cookieHeader) {
    return headers;
  }
  return {
...
export async function loginWithPassword({ username, password, signal, timeoutMs = 30000 }) {
  ...
  if (!response.ok) {
    throw new Error(await readErrorMessage(response, "invalid_credentials"));
  }

  return response.json();
}

export async function logoutCurrentUser({ signal, timeoutMs = 30000 } = {}) {
  ...
  if (!response.ok) {
    throw new Error(await readErrorMessage(response, "logout_failed"));
  }

  return response.json();
}

export async function fetchCurrentUser({ cookieHeader = "", signal, timeoutMs = 30000 } = {}) {
  const response = await fetchWithTimeout(`${API_BASE_URL}/api/v1/me`, {
    method: "GET",
    cache: "no-store",
  ...
  if (!response.ok) {
    throw new Error(await readErrorMessage(response, "unauthenticated"));
  }

  return response.json();
}

export async function fetchWorkspaces({ cookieHeader = "" } = {}) {
  try {
...
export async function fetchRuns({ cookieHeader = "" } = {}) {
  try {
...
export async function fetchRun(runId, { cookieHeader = "" } = {}) {
  const runs = await fetchRuns({ cookieHeader });
  return runs.find((item) => item.id === runId) || null;
}

export async function fetchRunMemory(runId, { cookieHeader = "" } = {}) {
  try {
