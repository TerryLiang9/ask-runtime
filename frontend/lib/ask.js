...
export function isOptionalFeishuSetupMessage(message) {
  if (!message) {
    return false;
  }
...
export function formatAskErrorMessage(message) {
  ...
  if (isOptionalFeishuSetupMessage(message)) {
    return "飞书集成未配置；普通 Ask、知识检索和文档功能可以继续演示。";
  }
...
export function buildOptionalFeishuBindingFromError(message) {
  return {
    status: "UNAVAILABLE",
    identity: {},
    required_scopes: [],
    missing_scopes: [],
    hint: formatAskErrorMessage(message),
  };
}
...
export function formatFeishuBindingHint(binding) {
  if (!binding) {
    return "飞书是可选集成；普通 Ask、知识检索和文档功能可以继续演示。";
  }
  if (binding.status === "UNBOUND") {
    return binding.hint || "飞书尚未绑定；普通 Ask、知识检索和文档功能可以继续演示。";
  }
  if (binding.status === "REAUTH_REQUIRED") {
    return (
      binding.hint ||
  ...
  if (binding.status === "UNAVAILABLE") {
    return binding.hint || "飞书集成未配置；普通 Ask、知识检索和文档功能可以继续演示。";
  }
...
export function getFeishuBindingBannerClass(binding) {
  if (!binding || binding.status === "UNBOUND" || binding.status === "UNAVAILABLE") {
    return "status-idle";
  }
  const missingScopes = binding.missing_scopes || [];
  if (binding.status === "ACTIVE" && !missingScopes.length) {
    return "status-idle";
  }
  return "status-error";
}
